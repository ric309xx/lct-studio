# 專案執行日誌 (Project Execution Log)

此文件用於記錄專案的每一次執行、變更與迭代。
請 Agent 在每次任務結束時，將重要變更記錄於此。

## [2026-08-20] Viewer 比較模式效能與視角保留
- 三模型與 OBJ／B3DMS 比較改為在既有 Cesium Viewer 中只替換被選取側的 tileset；底圖、地形、相機與另一側模型不重建，切換後維持原視角。
- 左右交換改為直接交換既有 tileset 與 split direction，不重新載入地圖或模型。
- 比較模式採較平衡的渲染設定：SSE `20`、dynamic SSE、漸進解析度 `0.45`、移動時請求裁切、`256 MB` cache／`128 MB` overflow，並停用模型陰影；單模型專案維持原品質設定。
- TypeScript 與正式版 Vite 建置通過，並同步 Viewer bundle 與 Cesium runtime 至本機網站。
- 本機瀏覽器確認切換與交換過程維持同一個 Cesium canvas；目前本機 token 無法讀取比較 Asset，因此完整模型畫面仍需在具正確 Cesium ion 授權的來源網域複驗。
- Vitest 因既有 `expect-type` 套件缺少 `./branding` 檔案而無法啟動，與本次 Viewer 程式變更無關。

## [2026-08-03] 新增瑞芳邊坡 OBJ 專案並修正 Cesium 靜態資源路徑
- 新增「新北市瑞芳區邊坡測試_OBJ」，使用 Cesium ion Asset `5107551`，並建立隱藏入口 `/3d-viewer/p/L5cT7iQ1nA9x/`；入口只保存既有格式的密碼雜湊，不在公開文件記錄明文。
- 設定專案預設視角為經度 `121.84313649`、緯度 `25.12754186`、高度 `120.01`、heading `141.554`、pitch `-21.041`、roll `0`。
- 本機 VS Code Live Server 固定使用 `5500`；Cesium ion Viewer token 已確認允許 `127.0.0.1:5500`，且目前資源範圍為 All assets。
- 修正發布建置漏帶 `VITE_BASE_PATH=/3d-viewer/` 導致 Cesium Workers／WASM 從錯誤根路徑載入、所有專案停止繪製的問題。
- CesiumJS `1.143.0` runtime 改放在版本化路徑 `3d-viewer/cesium-1.143.0/`，避免瀏覽器沿用舊版 Worker 快取。
- ESLint、17 項測試、TypeScript、正式建置及本機瀏覽器驗證通過；既有林口專案與新瑞芳專案均可正常載入。

## [2026-08-01] 新增三峽太陽能板2專案並發布
- 新增「新北市三峽區太陽能板2」，使用 Cesium ion Asset `5105006`，正式專案總數由 2 個增為 3 個。
- 建立獨立隱藏入口 `/3d-viewer/p/T7nV2qL9bX4m/`、個別密碼雜湊與單一專案觀看隔離；公開文件不記錄密碼明文。
- 同步更新 Local Uploader 的 `project-data/projects.json`、產生的 `src/projects.ts`、登入頁與 README，並以正式建置輸出更新網站 Viewer bundle。
- Local Uploader 依賴曾損壞，已移至 `viewer_node_modules_broken_20260801` 備份並依鎖檔重建；ESLint、16 項測試、TypeScript 與正式版建置通過。
- 目前 Viewer token 查詢 Asset `5105006` 回傳 `403 Forbidden`；需在 Cesium ion 將該 Asset 加入 token 的 Selected Assets 後，才能完成模型載入驗證。
- 依使用者指示，一併刪除不再使用的照片 `public/photos/城市光影/新北土城天好運2(2).jpg`。

## [2026-07-29] 手機版專案面板收合
- 在 760px 以下畫面加入專案面板「收合／展開」按鈕。
- 收合後面板縮為左側 48px 按鈕，釋放模型瀏覽空間；桌面版維持原版面。
- 補強本機開發模式的管理員／一般觀看切換，方便手機測試；正式環境仍由 1111／1234 密碼入口決定角色。
- 更新 Viewer JS／CSS bundle，並移除被取代的舊版資源。
- ESLint、14 項測試、TypeScript 與正式版建置均通過。

## [2026-07-29] 一般觀看模式套用模型範圍
- 修正模型範圍僅在管理員模式讀取的問題。
- 1234 一般觀看模式現在會套用同一瀏覽器內由 1111 管理員儲存的範圍，但仍無法編輯或清除。
- 瀏覽器 localStorage 不會跨裝置共享；跨訪客的正式發布範圍需改為專案設定或後端儲存。
- ESLint、14 項測試、TypeScript 與正式版建置均通過。

## [2026-07-29] 3D Viewer 固定使用真實地形
- 移除側欄「地形」區塊與「真實地形／平面」切換按鈕。
- Viewer 啟動時固定載入 Cesium World Terrain；載入失敗時顯示 token 權限提示。
- 更新正式站 Viewer bundle，並移除被取代的舊版 bundle。
- ESLint、14 項測試、TypeScript 與正式版建置均通過。

## 格式範例
```markdown
### [YYYY-MM-DD] 任務名稱
- **變更類型**: (功能新增 / 修復 / 優化 / 文件)
- **詳細內容**:
  - 項目 1
  - 項目 2
- **下一步**: (選填)
```

---

## [2026-06-28] 服務項目卡片出現動畫放慢
- **更新類型**: 動畫微調 / 本機修改
- **主要變更**:
  - 服務項目卡片動畫時間由 `1.15s` 調整為 `1.55s`。
  - 四張卡片的階梯延遲調整為 `0s / 0.42s / 0.84s / 1.26s`，讓出現節奏更慢。
  - 更新本機快取版本至 `style.css?v=69`。
- **備註**:
  - 依使用者要求，本次修改暫不自動上傳。

## [2026-06-28] 照片產生腳本保護 assets 與地圖快速編輯
- **更新類型**: 修復 / 編輯流程優化 / 本機修改
- **主要變更**:
  - 修正 `generate_photo_list.py`，現在只清理與重建 `public/photos`，不再刪除或重建 `public/assets`。
  - 補回被照片產生腳本誤刪的 `public/assets` 網站固定素材。
  - 空拍地圖 `?editMap=1` 編輯模式新增快速選取標示、作品照片搜尋、新增照片標示、套用照片到目前標示。
  - 地圖標示資料新增「新北土城工業區日落」，並更新 cache 至 `map_markers.js?v=4`、`main.js?v=67`。
- **備註**:
  - 依使用者要求，本次修改暫不自動上傳。

## [2026-06-28] 日夜對比英文標籤修正
- **更新類型**: 文案修正 / 本機修改
- **主要變更**:
  - 修正前後影像對比區塊的英文標籤，左側夜景改為 `Night`，右側日景改為 `Day`。
  - 移除對比區標題外層的 `reveal` class，避免標題區因動畫狀態未觸發而保持透明。
  - 評估此區塊上方已有「前後影像對比」標題與說明，因此不另外新增重複標題。
- **備註**:
  - 依使用者要求，本次修改暫不自動上傳。

## [2026-06-28] 空拍地圖澎湖點位照片修正
- **更新類型**: 修復 / 本機修改
- **主要變更**:
  - 修正空拍地圖澎湖點位原本指向不存在的 `澎湖湖西菓葉觀日樓.jpg`，導致預覽退回顯示地圖底圖。
  - 澎湖點位改為目前照片資料中存在的 `澎湖後寮天堂路  (2).jpg`，標題同步改為「澎湖後寮天堂路」。
  - 更新本機快取版本至 `map_markers.js?v=3`。
- **備註**:
  - 依使用者要求，本次修改暫不自動上傳。

## [2026-06-28] 線上畫冊桌機版尺寸放大調整
- **更新類型**: 版面微調 / 本機修改
- **主要變更**:
  - 將線上畫冊桌機版頁面上限由 `70vh` 調整為 `82vh`，讓照片與畫冊紙張接近預覽框可用範圍。
  - 桌機版 slide 外距由 `md:p-8` 改為 `md:p-4`，內容白邊由 `md:p-10` 改為 `md:p-6`，放大照片但保留畫冊留白。
  - 更新本機快取版本至 `main.js?v=66`。
- **備註**:
  - 依使用者要求，本次修改暫不自動上傳。

## [2026-06-28] Git 自動上傳腳本與首頁素材保護修正
- **更新類型**: 修復 / 上線流程 / 文件
- **主要變更**:
  - 修正 `git_auto.py` 在目前工作分支沒有 upstream 時，單純執行 `git push` 會失敗的問題。
  - 上傳腳本改為固定推送 `git push origin HEAD:main`，讓目前分支的最新提交可直接更新正式站。
  - 上傳前新增首頁必要素材檢查，若 logo、日夜對比圖、地圖底圖、服務項目圖或故事圖不存在，會停止上傳，避免正式網站破圖。
  - 補回前一次照片提交中被誤刪、但 `index.html` 仍在引用的首頁必要圖片素材。
- **驗證**:
  - `python -m py_compile git_auto.py`

## [2026-06-28] 空拍地圖點位預覽切換優化
- **更新類型**: 修復 / 效能優化
- **主要變更**:
  - 修正空拍地圖標示點位切換時，預覽圖會先停在上一張或短暫顯示其他點位照片的問題。
  - 移除點位切換時的固定延遲，改為立即更新圖片路徑，並用請求序號避免舊圖片載入完成後覆蓋新點位。
  - 新增點位預覽圖片背景預載，降低第一次點選不同點位時的等待感。
  - 更新前端 cache 版本至 `main.js?v=65`。
- **驗證**:
  - `node --check public/js/main.js`

## [2026-06-27] 精選影片版型、日夜對比、服務項目與地圖編輯模式上線
- **更新類型**: 首頁版型 / 互動功能 / 服務項目 / 地圖資料 / GitHub 上線
- **主要變更**:
  - 精選影片改為大型主視覺播放器搭配 4 個影片縮圖，修正播放其他影片後原主影片消失的問題。
  - 影片年份改為依資料顯示 `2025` / `2026`，並移除主影片說明文字以保持畫面乾淨。
  - 在精選影片前新增「前後影像對比」區塊，使用 `public/assets/compare/linkou-day.jpg` 與 `public/assets/compare/linkou-night.jpg` 做日夜滑桿對比。
  - 新增 `?editCompare=1` 對比圖調整模式，可調整日照/夜照的縮放、左右、上下位置，最後固定為 `data-day-x="33" data-day-y="53" data-day-zoom="109"`、`data-night-x="31" data-night-y="48" data-night-zoom="112"`。
  - 新增「服務項目」區塊，四項為「建築形象空拍」、「工程進度紀錄」、「活動影像紀錄」、「景點旅遊宣傳」，並使用 `public/assets/services/` 圖片。
  - 空拍地圖標示改為獨立資料檔 `public/js/map_markers.js`，正式站預設隱藏編輯工具；需使用 `?editMap=1` 才顯示新增、修改、刪除、複製設定工具。
  - 更新前端 cache 版本至 `style.css?v=67`、`main.js?v=63`，並新增 `map_markers.js?v=2`。
- **驗證**:
  - `node --check public/js/main.js`
  - Codex 內建瀏覽器檢查 `http://localhost:5173/index.html` 與 `?editCompare=1`，確認新區塊、服務文字、地圖編輯隱藏/編輯模式正常。
- **上線注意**:
  - 正式地圖編輯入口：`/index.html?editMap=1`
  - 日夜對比調整入口：`/index.html?editCompare=1`
  - 工作區仍有既有未處理檔案狀態：`public/assets/logo-2.png` 刪除狀態、`public/assets/logo (1).png`、`public/assets/logo (2).png`、`scroll-animation-technique.md` 未追蹤；本次不納入上傳。

## [2026-06-27] 首頁效能與圖片載入優化
- **更新類型**: 效能優化 / 圖片壓縮 / 載入策略
- **主要變更**:
  - 將日夜對比圖由 3840x2160 壓縮為 1920x1080 漸進式 JPEG：
    - `public/assets/compare/linkou-day.jpg` 約 6.1MB -> 約 489KB
    - `public/assets/compare/linkou-night.jpg` 約 4.0MB -> 約 286KB
  - 將首頁故事段落大圖轉為 WebP：
    - `story-sunrise-qingjing.webp`
    - `story-morning-qingjing-cloudsea.webp`
    - `story-golden-xiluo-silhouette.webp`
    - `story-sunset-yilan-wujie.webp`
  - 將空拍地圖底圖改為 `public/assets/taiwan-aerial-map.webp`，並同步更新 JS fallback。
  - 移除已被 WebP 取代的舊大型 PNG/JPG 發布資產。
  - 為故事圖、地圖、日夜對比圖、服務項目圖補上 `loading="lazy"` 與 `decoding="async"`。
  - 對 `cdn.jsdelivr.net`、Google Fonts、YouTube 圖片與 YouTube iframe domain 補上 preconnect。
  - 底部 CDN 與本地 JS script 改為 `defer`，並更新 cache 至 `style.css?v=68`、`main.js?v=64`。
- **驗證**:
  - `node --check public/js/main.js`
  - `node --check public/js/data_photos.js`
  - `node --check public/js/map_markers.js`
  - 確認首頁程式不再引用舊故事 PNG/JPG 與舊地圖 PNG。

## [2026-01-26] Agent Skills 設定與畫廊修復
- **變更類型**: 修復 & 文件
- **詳細內容**:
  - **修復畫廊跳頁問題**: 修正 `public/js/main.js` 中重複綁定 Event Listener 導致點擊下一張時 index 跳號的問題。
  - **建立 Agent Skills**: 建立 `.cursorrules` 檔案，定義專案開發準則：
    1.  **語言**: 強制使用繁體中文。
    2.  **日誌**: 強制維護 `PROJECT_LOG.md`。
    3.  **手冊**: 強制維護 `PROJECT_HANDBOOK.md` (流程與復盤)。
    4.  **權限**: (User Added) 禁止擅自安裝套件；核心文件修改需先提案。
- **下一步**: 確保後續開發遵循此規範。

## [2026-01-19] 功能歸檔 (Feature Archiving)
- **變更類型**: 優化
- **詳細內容**:
  - **3D 懸浮藝廊歸檔**: 將 v15 版本的 3D 藝廊功能相關程式碼從 `index.html` 與 `main.js` 移除，並備份至 `ARCHIVE_3D_GALLERY.md`。

## [2026-01-15] 響應式畫冊與安全性 (Responsive & Security)
- **變更類型**: 功能新增 & 優化
- **詳細內容**:
  - **響應式畫冊**: 實作 `isMobile` 偵測。手機版改為 A4 直式排版 (Vertical Stack)，電腦版維持 A3 橫式。
  - **白邊控制**: 手機版強制圖片高度 43% 以確保留白。
  - **手機體驗**: 修復螢幕旋轉時的輪播錯位 (Swiper Observer)。
  - **安全性**: 加入防右鍵、防 F12、以及 Domain Locking。

## [2026-01-14] 畫冊介面微調 (UI Fixes)
- **變更類型**: 修復
- **詳細內容**:
  - **封面響應式**: 修復平板模式下 "PORT-FOLIO" 標題溢出問題。
  - **排版微調**: 縮減封面內距。

## [2026-01-13] 視覺重製與線上畫冊 (Visual Overhaul)
- **變更類型**: 視覺重構 & 功能新增
- **詳細內容**:
  - **關於我重製**: 轉型為「高科技極簡主義」，使用螢光青色 (`#00F0FF`) 與淺灰藍色 (`#BEBEBE`)。排版改為雙欄 + 拍立得裝飾。
  - **線上畫冊模式**: 更名並轉型為「極簡藝廊風格 (Light Gallery)」，背景改為米白色 (`#F9F9F9`)，圖片加入圓角與懸浮陰影。
  - **系統優化**:
    - **浮水印**: 透明度調降至 30%。
    - **檔名清洗**: Python 腳本加入自動去空白與轉小寫功能，解決 404 問題。
    - **效能**: 畫冊圖片加入 Lazy Loading，JPEG 壓縮至 70%。
  - **Late Night Update**: 新增單頁式雜誌封面 (A4/A3)，優化燈箱導覽邏輯。

## [2026-06-09] 首頁電影感滾動體驗升級
- **變更類型**: 視覺優化 / 功能新增 / SEO / 上線
- **詳細內容**:
  - **Hero 改版**:
    - 加入 GSAP + ScrollTrigger，讓背景影片在滾動時產生慢速推鏡與淡出效果。
    - 新增暗角、膠片顆粒、滾動提示線，提升首頁電影感。
    - 主標題更新為「以空中視角，看見地景的壯闊與細節之美」，並手動斷行為第一行較長、第二行較短。
    - 副標更新為「從城市紋理、自然地貌到工程現場，記錄每一個值得被看見的瞬間。」
    - 修正 Hero 文字往下滑再回頂部後不顯示的問題：移除 Hero 的 `.reveal`，並在 `ScrollTrigger.onLeaveBack` 強制恢復 opacity 與 yPercent。
  - **作品照片區**:
    - `城市光影`、`大地映像` 維持固定網格位置，不再套用照片卡滾動進場、錯位或視差動畫。
    - 保留原本 hover 放大與燈箱瀏覽功能。
  - **精選影片區**:
    - 補強「精選影片」標題區，改為固定顯示，不套用 `reveal` 或 section-intro 的 GSAP 動畫，避免滑動後標題消失。
  - **服務案例區**:
    - 新增導覽項目「服務案例」。
    - 新增空拍攝影、工程紀錄、建案視覺、太陽能場域四個商業應用卡片，作為未來接案與太陽能網站設計的展示基礎。
  - **SEO 與維護性**:
    - 刪除 `_headers`，移除 `X-Robots-Tag: noindex`，讓正式網站可被搜尋引擎收錄。
    - 移除右鍵鎖定與 anti-debugger 迴圈，改成輕量網域提示，避免影響維護、除錯與使用體驗。
  - **上線紀錄**:
    - Commit: `311d6f9 升級首頁電影感滾動體驗`
    - Push: `origin/main`
- **驗證**:
  - `node --check public/js/main.js`
  - `node --check public/js/data_photos.js`
  - `node --check public/js/data_videos.js`
  - 自訂檢查：Hero 文案、服務段落、GSAP CDN、手機選單、noindex 移除、照片區固定顯示條件皆通過。
- **下一步**:
  - 線上站更新後，實際檢查 `https://lctstudio.tw/` 的 Hero、作品照片、影片標題、服務案例與手機版排版。
  - 若要使用 AI 影片，優先作為太陽能/廠房/能源場域的氣氛型 Hero 短片；LCT 本站仍建議以真實空拍素材為主。
---

## [2026-06-11] Sun Path / Aerial Light 滾動敘事段落與首頁視覺收斂
- **變更類型**: 互動設計 / 首頁視覺 / 作品集展示 / 筆記更新
- **主要變更**:
  - 新增 `#scroll-story` 日光敘事段落，使用 `SUNRISE`、`MORNING LIGHT`、`GOLDEN HOUR`、`SUNSET` 四階段建立空拍分鏡。
  - 使用 GSAP ScrollTrigger + MotionPathPlugin，讓太陽沿弧線移動，並同步切換階段標籤、文字與背景照片。
  - 新增四張首頁敘事素材：
    - `public/assets/story-sunrise-qingjing.png`
    - `public/assets/story-morning-qingjing-cloudsea.png`
    - `public/assets/story-golden-xiluo-silhouette.jpg`
    - `public/assets/story-sunset-yilan-wujie.png`
  - 將作品、影片、服務、關於、聯絡區改成共用連續深色背景，移除造成段落分界明顯的色塊與上下遮罩。
  - 調整 `#about` 與 `#services` 的段落間距，移除關於我上方過大的空白。
  - 調整日落照片亮度：降低日落階段 overlay，並提高日落照片顯示透明度。
  - 修正日出故事圖路徑，從不存在的 `story-sunrise-qingjing-a.png` 改為 `story-sunrise-qingjing.png`。
  - 更新黃金時刻與日落階段文案：
    - 黃金時刻：`透過黃金時刻的柔和光線，讓建築、工程與場域細節，在影像中被完整呈現。`
    - 日落：`當光線逐漸收斂，場域的輪廓與情緒，也被整理成更有記憶點的影像。`
  - 校正背景大字 `.story-rail` 的位移，讓最後階段對齊 `SUNSET`。
  - 版本快取更新至 `style.css?v=41` 與 `main.js?v=41`。
- **驗證**:
  - `node --check public/js/main.js`
  - 確認 `public/assets/story-sunrise-qingjing.png`、`public/assets/story-sunset-yilan-wujie.png` 存在。
- **備註**:
  - Sun Path 四張故事圖屬於首頁敘事素材，不會由 `generate_photo_list.py` 自動產生。
  - 未來若替換敘事圖，建議先裁成網站用寬版圖，減少依賴 `object-position`。

---

## [2026-06-12] Sun Path 微調工具、導覽與段落小標更新
- **更新類型**: 視覺微調 / 文案更新 / 前端工具 / GitHub 上線
- **主要調整**:
  - 導覽列與手機選單的「服務案例」改為「影像服務」，讓該段更符合目前內容定位。
  - `Sun Path / Aerial Light` 四階段文案更新：
    - 01 日出｜喚醒地景的第一道光
    - 02 晨光｜視角攀升，層次隨之展開
    - 03 黃金時刻｜捕捉光影的最佳純度
    - 04 日落｜餘暉凝結，成就雋永影像
  - 背景大字改為完整 `SUNRISE / MORNING LIGHT / GOLDEN HOUR / SUNSET`。
  - 背景大字間距與頭尾對齊改為 CSS 可調參數：
    - `--story-rail-start-x: 2.5vw`
    - `--story-rail-base-gap: 2.5rem`
    - `--story-rail-gap-sunrise: 40vw`
    - `--story-rail-gap-morning: 12vw`
    - `--story-rail-gap-golden: 6vw`
    - `--story-rail-end-x: -1vw`
  - 新增 `?railTune=1` 調整面板，可在網站畫面上用滑桿即時調整背景大字的位置與間距，調整結果可直接複製回 CSS。
  - 桌機版太陽尺寸縮小，並重新設計高光、內陰影與外圈光暈，使太陽更精緻且不過度搶畫面。
  - 手機版太陽縮小，四個階段標籤移到下方，降低與主標題互相遮擋的情況。
  - 補上橘黃色英文小標：
    - 城市光影：`City Lights`
    - 大地映像：`Earthscapes`
    - 關於我：`About LCT`
    - 聯絡我：`Contact`
  - 更新前端 cache 版本至 `v=53`。
- **驗證**:
  - `node --check public/js/main.js`
- **備註**:
  - `?railTune=1` 僅供調整使用，正常網址不會顯示調整面板。
  - 工作區仍有既有未處理檔案狀態：`public/assets/logo-2.png` 刪除狀態、`public/assets/logo (1).png`、`public/assets/logo (2).png`、`scroll-animation-technique.md` 未追蹤；本次不上傳這些檔案。
# [2026-07-24] 活動影片精品燈箱與前端安全補強
- **更新類型**: 服務項目互動 / YouTube 播放 / 安全性 / 無障礙
- **內容**:
  - 「活動影像紀錄」改為可點擊卡片，新增活動影片精品燈箱與資料驅動清單。
  - 新增水樂園活動影片，活動作品集中於 `data_videos.js` 的 `events` 分類。
  - YouTube 網址改為驗證影片 ID 後使用 `youtube-nocookie.com`，iframe 延遲至使用者按下播放才建立。
  - 燈箱支援 Esc、背景關閉、焦點循環、關閉後焦點還原與背景捲動鎖定。
  - 動態照片卡片改用 DOM API，並補上相容現有第三方服務的 HTML CSP allowlist。
  - 更新首頁 cache query；本次未執行 Git commit 或 push。
  - 服務項目「景點旅遊宣傳」調整為「旅遊航拍紀錄」，文案聚焦地景、路線與目的地氛圍，與活動現場紀錄做出區隔。
  - 新增 `tourism` 影片分類與兩支旅遊 Shorts，並讓活動、旅遊卡片共用同一套資料驅動精品燈箱。
  - 優化 Sun Path 滾動敘事：移除 CSS sticky 與 GSAP pin 的重複定位、縮短 scrub 追趕時間、將階段動畫收斂為 GPU 友善的 transform／opacity，並提前解碼四張故事背景圖。
# [2026-07-29] 3D GIS 展示功能與網站整合
- **更新類型**: 3D 模型展示 / 工程量測 / 首頁整合 / 效能優化
- **內容**:
  - 首頁新增「可分享、可量測、可分析的 3D GIS 成果」展示區塊。
  - 右側示範影片改為 720p 版本，使用 IntersectionObserver 進入視窗附近才載入，並停用影片區塊的點擊與控制，避免誤導覽。
  - 導覽列移除重複的「3D 模型」項目，保留展示區塊的「開始瀏覽 3D 模型」入口。
  - 新增 `/3d-viewer/` 靜態 Viewer，支援衛星航照／道路圖、真實地形／平面地球、專案切換、距離與面積量測、量測清單定位、分享連結、回到模型、重新載入及全螢幕。
  - 量測成果最多各保留 3 組；點選面積清單會飛回該範圍並以紅色強調。
  - 提供一般觀看與管理員兩種介面權限；管理員可框選模型顯示範圍及複製相機視角。
  - 清除 32 份未被頁面引用的舊版 Viewer bundle，共減少約 104 MB。
- **安全性與限制**:
  - 目前密碼驗證為純前端入口控管，只適合展示或避免一般訪客誤操作，不等同伺服器端身分驗證。
  - Cesium ion token 會被編譯進瀏覽器程式，正式 token 必須限制為必要的 `assets:read` 權限、指定 Asset 及允許網域。
  - Cesium Viewer 內的官方 attribution 必須保留；首頁行銷文案與影片不顯示底層技術名稱。
- **驗證**:
  - Viewer TypeScript 正式版建置。
  - Viewer 單元／元件測試。
  - 首頁 JavaScript 語法、連結、影片與靜態資源路徑檢查。
- **部署備註**:
  - GitHub Pages 使用 `/3d-viewer/` 子路徑，建置時需設定 `VITE_BASE_PATH=/3d-viewer/`。
  - 正式上線前應在 Cesium ion token 的 Allowed URLs 加入正式網站來源。

# [2026-07-31] 3D 專案隱藏網址與個別密碼分享
- **更新類型**: 專案分享 / 前端權限隔離 / Viewer 部署維護
- **內容**:
  - 分享流程改為「隱藏專案網址＋個別密碼」，官網與客戶登入頁不公開列出全部專案。
  - 每個客戶入口只對應單一專案；驗證成功後不顯示其他專案選項。
  - 管理員入口保留完整專案切換、模型範圍框選、複製視角及專案分享。
  - 「分享連結」改為複製目前專案的隱藏網址，網址不包含密碼，密碼需另外提供。
  - 專案入口加入 `noindex, nofollow, noarchive`，降低被搜尋引擎建立索引的機率。
  - Viewer 正式資源改為固定檔名 `assets/viewer.js` 與 `assets/viewer.css`，後續建置不再需要手動更新入口引用的雜湊檔名。
  - 不同入口使用各自的瀏覽工作階段，管理員登入不會自動解鎖客戶專案入口。
- **驗證**:
  - Viewer 共 16 個單元／元件測試通過。
  - ESLint、TypeScript 與 Vite 正式版建置通過。
  - 管理員入口、兩個專案入口及固定 Viewer 資源在獨立本機測試埠皆回傳 HTTP 200。
  - 確認三組密碼雜湊、專案隔離、`noindex` 與分享網址對應正確。
- **安全性與限制**:
  - 目前網站為純靜態部署，隱藏網址與前端密碼只能降低一般訪客誤入風險，不是機密資料的真正存取控制。
  - 線上停用連結、修改密碼、重新產生網址與查看存取紀錄，需加入後端 API 與伺服器端授權後才能安全實作。

# [2026-08-11] 數位地景典藏與南雅奇岩
- **更新類型**: 公開數位典藏 / Cesium 3D / Viewer 專案與密碼維護
- **內容**:
  - 首頁新增精品風格「數位典藏」入口，建立獨立 `/archive/` 頁面。
  - 第一階段收錄南雅奇岩 Asset `5124638` 與龍洞一般掃描 `5119307`。
  - 典藏頁提供地點切換、模型飛行模擬、密碼保護的互動 3D 入口，以及南雅、龍洞各五張照片燈箱。
  - Viewer manifest 新增 `20260810 南雅奇岩`，分享入口為 `/3d-viewer/p/Y8nA4rK2mP7x/`。
  - 南雅奇岩 Viewer 預設相機更新為經度 `121.89251409`、緯度 `25.11970394`、高度 `69.3m`、heading `56.958°`、pitch `-39.588°`、roll `0°`。
  - 所有專案與比較入口統一使用觀看者密碼規則，管理員入口維持獨立密碼；公開文件不記錄明文密碼。
  - 首頁 CSS cache query 更新至 `v=77`。
  - 重新執行 `generate_photo_list.py`，新增 5 張南雅奇岩照片並更新照片資料清單；南雅與龍洞代表封面最終使用各自第 4 張與第 5 張照片。
  - 典藏頁改為先播放模型飛行模擬影片，Cesium 互動模型移至右下角密碼入口；移除模型版本與上方量測工具，後續主題影片採資料陣列擴充。
  - 將原本獨立的「開始瀏覽 3D 模型」展示段落整合進數位典藏，改為功能索引與低調的私人專案管理入口，避免首頁出現兩個重複的 3D 主題區塊。
  - 數位典藏介紹統一留在首頁，內頁移除重複的大型開場並直接進入地點內容；主標改為「保存文化資產，延續地景的空間記憶。」並縮小字級。
  - 首頁典藏入口進一步整合為單一功能介紹影片畫面，不在首頁提前展示個別案場；專案清單集中於 `/archive/`。
  - 桌機與手機導覽列的「數位典藏」改為首頁區段錨點；使用者先查看功能介紹，再由區段內按鈕進入專案清單。
  - 修正 localhost 分享專案被開發模式誤判為管理員：Viewer 載入前會將受限專案 ID 寫入網址上下文，確保觀看密碼維持單一專案權限；只有管理員驗證可切換管理模式。
  - 修正分享頁由 viewer 切換管理員後又被 Gate 降回 viewer 的問題；已通過驗證的管理員工作階段在重載時保留。
  - 水湳洞選煉廠遺址串接 Cesium ion Asset `5125623`；典藏右側欄位改呈現地景特色、保存內容與瀏覽重點，不再顯示模型技術資訊。
  - 重新執行 `generate_photo_list.py`，新增 2 張水湳洞選煉廠遺址照片，並將其加入第三個公開典藏地點；後續已串接 Cesium Asset `5125623` 與專案分享入口。
  - 修復 OneDrive 原圖在受限程序中無法讀取造成的封面缺檔，已確認南雅與龍洞各 5 張、水湳洞 2 張公開照片完整輸出。
- **安全性與限制**:
  - 公開典藏免密碼；工程與客戶專案仍只透過原本的分享入口提供。
  - 前端密碼只屬簡易門禁；Viewer 的 Cesium token 必須限制可讀 Asset 與 Allowed URLs。
  - 本次只完成本機檔案，未執行 Git add、commit、push 或部署。
- **驗證紀錄**:
  - Viewer manifest 產生器成功建立 9 個專案與分享入口。
  - Viewer 現有依賴安裝有缺檔：Vitest 缺少 `expect-type/branding`，Vite 缺少 Cesium shader，TypeScript 另有既有型別錯誤；因此改以既有正式 bundle 做最小資料同步，並保留問題待依賴環境修復。

# [2026-08-17] 數位典藏內容、影片與照片精選更新
- **更新類型**: 數位典藏 / YouTube 功能導覽 / 公開案場 / Viewer 權限 / 影像精選
- **內容**:
  - 首頁數位典藏區改為四項單列功能導覽：三維模型展示、不同時期／格式比較、日照模擬、距離／面積量測；點選項目即切換並播放對應 YouTube 影片。
  - 強化「進入數位典藏」入口與功能項目可讀性，統一典藏標題層級，移除影片畫面上重複的功能標籤。
  - 典藏內頁整合南雅奇岩、龍洞岩場與水湳洞選煉廠遺址的飛行模擬影片、照片故事及互動三維模型入口。
  - 官網的三維模型入口維持管理員驗證；一般觀看者只能透過管理員產生的個別分享網址與專案密碼進入，且不同專案不共用觀看授權。
  - 更新水湳洞預設相機視角，並整理龍洞三個單一模型與一個可切換左右資料源的比較專案。
  - 南雅奇岩照片由五張精選為第 1、3、4、5 張；龍洞岩場精選為第 1、2、4、5 張。來源照片不刪除，只調整典藏顯示清單。
  - `archive/archive.js` 新增每個地點最多四張照片的共通限制，後續案場應先挑除重複構圖再上架。
- **驗證**:
  - `node --check archive/archive.js` 通過。
  - 本機 HTTP 網站已用於典藏首頁、案場切換、影片與照片顯示調整。
  - 功能導覽第一項「不同時期／格式比較」更新為 YouTube 影片 `6Qb7HdAqiRA`；原第一項影片移至第二項並更名為「邊坡監測」。
  - 首頁初次進入數位典藏時改顯示龍洞代表照片，不預先載入 YouTube；使用者點擊封面後才載入並自動播放影片，避免先出現模型未載入的黑色畫面並減少初始流量。
# [2026-09-10] 三峽太陽能板切平面圖塊編修與 A4 PDF
- **更新類型**: Cesium 3D Viewer / 建物切平面 / 圖塊編修 / PDF 輸出
- **適用專案**: 新北市三峽區太陽能板_b3dms
- **內容**:
  - 建物水平切片完成後可輸出 A4 PDF，支援比例尺 1:100、1:200、1:500、1:600、1:1000、1:1200。
  - PDF 圖框製圖單位由「LCT Studio」簡化為「LCT」。
  - 平面輪廓編修新增圖塊選取狀態，選中圖塊以橘色線條標示。
  - 每個圖塊可個別刪除，並保留既有節點拖曳、增刪節點、圖塊切分與尺寸標註功能。
  - 新增「手繪取代」：選擇既有圖塊後重新描繪輪廓，完成時取代該圖塊。
  - 新增「新增圖塊」：保留現有輪廓並手動畫出額外圖塊。
  - 修正數位典藏影片播放後播放鍵仍持續顯示的問題。
- **驗證**:
  - Viewer 22 項單元／元件測試全部通過。
  - TypeScript 與 Vite 正式版建置成功，並以 `/3d-viewer/` 子路徑輸出部署檔。
  - VS Code 本機靜態預覽資源回傳 HTTP 200，確認新版 Viewer 包含新增、取代、刪除圖塊及「製圖 LCT」內容。
- **操作方式**:
  - 點選輪廓內部選擇圖塊，再使用「刪除圖塊」或「手繪取代」。
  - 使用「新增圖塊」依序點選至少三個轉角，完成後加入既有平面輪廓。

# [2026-09-12] 龍騰斷橋第四個數位典藏專案正式發布
- **更新類型**: 數位地景典藏 / 私人 Cesium Viewer / Cloudflare R2 / 影像與影片更新
- **內容**:
  - 將「龍騰斷橋模型」加入數位地景典藏，成為第 4 個公開專案。
  - 串接私人 Cesium Viewer 專案 `longteng-20260906`，模型資料使用伺服器端固定 R2 路徑與專案白名單。
  - Viewer 採模型座標推導的自動視角，呈現橋墩、拱圈、步道與周邊植被的完整範圍。
  - 影像紀錄由原本 4 張更新為 3 張指定照片，空拍全景作為影片封面。
  - 模型飛行模擬影片最終更新為 YouTube `MtW9TNcChTM`。
  - 保留既有南雅奇岩、龍洞岩場、水湳洞與舊 `/tiles` 路由的相容性。
  - 將舊版 Viewer 本機建置檔與帶電腦名稱的 HTML 備份加入 Git 忽略清單，正式資源仍使用 `3d-viewer/assets/viewer.js` 與 `viewer.css`。
- **驗證**:
  - Worker 測試 7 項通過，R2 tileset 與深層 `.b3dm` 資源完成 GET、HEAD、Range 與 MIME 檢查。
  - 正式典藏頁確認顯示第 4 個專案、3 張影像與私人模型入口。
  - 正式頁面播放後確認嵌入 `youtube-nocookie.com/embed/MtW9TNcChTM`。
  - GitHub `origin/main` 已包含龍騰斷橋典藏、照片與最終影片更新。
