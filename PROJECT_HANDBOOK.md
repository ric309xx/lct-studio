...
2. 收合狀態只存在目前頁面生命週期，重新整理後預設展開，確保使用者不會找不到專案控制。
...
- 首頁入口位於 `#digital-archive`，完整典藏頁位於 `/archive/`。
- 典藏定位與介紹文案只放在首頁；內頁直接從地點選單開始，避免首頁與內頁重複大型標題。
- 首頁只呈現一個與個別案場無關的數位典藏功能介紹影片；南雅奇岩、龍洞岩場與水湳洞選煉廠遺址等專案只在 `/archive/` 顯示。
- 第一階段公開地點為南雅奇岩、龍洞岩場與水湳洞選煉廠遺址；模型資料集中在 `archive/archive.js` 的 `sites` 陣列。
- 南雅奇岩使用 Cesium ion Asset `5124638`；龍洞公開入口使用一般掃描 `5119307`，不在典藏頁提供模型版本選單。
- 水湳洞選煉廠遺址使用 Cesium ion Asset `5125623`，Viewer 專案 ID 為 `ruifang-shuinandong-smelter-20260811`，分享入口為 `/3d-viewer/p/S5nD8gR2mK6x/`。
- 南雅與龍洞各精選四張照片；南雅使用第 1、3、4、5 張，龍洞使用第 1、2、4、5 張，水湳洞目前使用兩張照片。代表封面分別使用南雅第 4 張、龍洞第 5 張與水湳洞第 1 張。
- 每個典藏地點最多顯示四張影像；若來源超過四張，應先挑除構圖重複的照片，再於 `archive/archive.js` 以 `selectedPhotos()` 明確指定。

### B. 飛行模擬影片與互動模型

- 典藏頁的主畫面先播放模型飛行模擬影片，不會直接載入 Cesium，降低首頁負擔並讓觀看路徑更單純。
- 每個地點的影片集中在 `archive/archive.js` 的 `films` 陣列；新增主題影片時加入 `id`、`title`、`youtubeId`、`poster` 即可。
- 各案場飛行模擬使用 YouTube 影片 ID 串接；更換影片時只需修改對應 `youtubeId`。
- 右下角「進入三維模型平台」才連往該地點的密碼入口。典藏頁本身不保存或使用 Cesium token。

### C. Viewer 清單與密碼

- Viewer 原始 manifest 位於 `D:/OneDrive/Codex/12_Drone_GIS_Platform/viewer/project-data/projects.json`，南雅奇岩 ID 為 `ruifang-nanya-rock-20260810`。
- 專案／比較分享入口與管理員入口使用不同的 SHA-256 密碼雜湊；公開文件不得記錄明文密碼。
- 密碼為純前端簡易門禁，不可視為機密資料的真正授權機制。
- 修改 manifest 後依序執行 `pnpm projects:generate`、測試與正式建置，再同步 `dist` 與 `gate/p` 到主站 `3d-viewer/`。
- 驗證密碼時需使用全新工作階段或清除 `sessionStorage`，避免既有授權狀態繞過登入頁。
- 分享入口載入 Viewer 前必須保留 `project` 網址上下文；本機開發環境會據此區分受限專案與直接管理入口，避免觀看密碼被 localhost 開發捷徑誤判為管理員。
- Viewer 分享頁從一般觀看切換管理員後，Gate 不可在重載時把已通過驗證的角色降回 viewer；觀看密碼僅授權分享入口指定的單一專案。

### D. 本機驗收

- 以本機 HTTP server 開啟網站，不要直接使用 `file://`，避免影片、CSP 與資源路徑在不同瀏覽器產生差異。
- 檢查南雅／龍洞切換、模型飛行模擬、三維模型密碼入口、兩地各四張照片燈箱及手機版面。
- 正式上線前確認 Cesium ion 商業方案、儲存容量、官方 attribution 與 token Allowed URLs。
